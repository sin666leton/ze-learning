...
<div class="flex-shrink-0 flex flex-col gap-4">
    <div class="flex-shrink-0 flex flex-col -space-y-1 p-2">
        <div class="flex-1">
            <h2 class="text-xl">Mata pelajaran</h4>
        </div>
        <div class="flex-1">
        ...
        <div class="flex gap-4">
            <div class="flex-1 form-control">
                ...
                <select name="classroom_id" id="classroom" class="bg-gray-100">
                    <option value="{{$subject['classroom_id']}}">{{$subject['classroom']['name']}}</option>
                </select>
            </div>
            <div class="flex-1 form-control">
                <label for="semester">Semester</label>
                <select name="semester_id" id="semester">
                    @foreach ( $subject['classroom']['academic_year']['semesters'] as $item )
                        @if ($item['id'] == $subject['semester_id'])
