        ...
        <div class="flex gap-4">
            <div class="flex-1 form-control">
                ...
                @if (request()->type == 'assignment')
                    <input type="text" class="bg-gray-100" name="" id="" value="{{$score->scoreable->studentClassroom->student->user->name}} - {{$score->scoreable->studentClassroom->student->nis}}" disabled>
                @else
                    <input type="text" class="bg-gray-100" name="" id="" value="{{$score->studentClassroom->student->user->name}} - {{$score->studentClassroom->student->nis}}" disabled>
                @endif
            </div>
            @if (request()->type == 'assignment')
                <div class="flex-1 form-control">
                ...
                <input type="hidden" name="belongsTo" value="{{ $score->scoreable->assignment_id }}">
                @else
                <div class="flex-1 form-control">
