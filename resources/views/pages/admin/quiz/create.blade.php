...
<div class="flex flex-col bg-white w-full rounded-md overflow-hidden shadow-md p-4">
    <form action="/admin/quizzes" method="post" class="form-group">
        @csrf
        <div class="form-control">
            <label for="subject">Mata pelajaran</label>
            <input type="text" name="subject" id="" value="{{$subject->name}}" disabled>
        ...
        <div class="form-control">
            ...
            <textarea name="content" id="description">{{old('content')}}</textarea>
        </div>
        <div class="flex-1 form-control">
            <label for="size">Durasi pengerjaan (menit)</label>
            <input type="number" name="duration" id="size" min="1" placeholder="contoh: 10" value="{{old('duration')}}">
        </div>
        <div class="flex flex-col items-center justify-between md:flex-row gap-3">
            <div class="flex-1 form-control">
                <label for="access_at">Dapat diakses pada</label>
                <input type="datetime-local" name="access_at" id="access_at" value="{{old('access_at')}}">
            </div>
            <div class="flex-1 form-control">
                <label for="expired_at">Berakhir pada</label>
                <input type="datetime-local" name="ended_at" id="expired_at" value="{{old('ended_at')}}">
            </div>
        </div>
