...
<div class="flex flex-col gap-2 justify-between bg-white rounded-md shadow-md mb-4">
    <div class="flex-shrink-0">
        <div class="flex items-center justify-between border-b border-gray-200 px-4 py-2 text-gray-500">
            <div class="flex-1">
                <span>{{$score['published'] ? 'Publikasi' : 'Tidak dipublikasi'}}</span>
            </div>
            <div class="flex-shrink-0">
    ...
    <div class="flex-1 px-4 py-2">
        <div class="flex flex-col gap-3">
            <div class="flex-shrink-0">
                <h4 class="truncate text-lg font-semibold text-blue-500">{{$score['student_classroom']['student']['user']['name']}}</h4>
            </div>
...
<div class="flex flex-col gap-4" id="dynamicField">
    @foreach ( $score['scoreable']['questions'] as $question )
        <div class="flex-shrink-0 flex flex-col bg-white w-full rounded-md overflow-hidden shadow-md gap-2">
            <div class="flex-shrink-0 flex items-center justify-between px-4 py-2 border-b border-gray-300">
                <div class="flex-1">
                    ...
                    @else
                        {{ $question['answer_question'][0]['is_correct'] ? "Benar" : "Salah" }}
                    @endif
                </div>
                <div class="flex-shrink-0">
                    Poin {{ $question['point'] }}
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <div class="flex-shrink-0">
                        @if ($question['answer_question'])
                            <p>{!! $question['answer_question'][0]['content'] !!}</p>
                        @else
                            <p class="text-red-500">Tidak dijawab</p>
                        @endif
                    </div>
                </div>
            </div>
